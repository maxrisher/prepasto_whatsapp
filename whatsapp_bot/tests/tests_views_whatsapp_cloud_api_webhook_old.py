...
class WhatsappWebhookTestCase(TestCase):
    ...
    def test_status_update(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_reaction_message_anonymous(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_reaction_message_user(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_normal_text_message_new_user(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_timezone_cancel(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
        ...
        self.assertEqual(messages[0].message_type, "NEW_USER_TIMEZONE_CANCELLATION")
        self.assertEqual(messages[1].message_type, 'PREPASTO_LOCATION_REQUEST_BUTTON')

    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_timezone_confirm(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_location_sharing(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_delete_meal_exists(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
        self.create_existing_user_setup()
        ...
        messages = WhatsappMessage.objects.filter(whatsapp_user=self.django_whatsapp_user).order_by('-record_created_at')
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_delete_meal_does_not_exist(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
        self.create_existing_user_setup()
        data = mock_whatsapp_webhook_data.delete_existing_meal_button_press
        data['entry'][0]['changes'][0]['value']['messages'][0]['interactive']['button_reply']['id'] = str(uuid.uuid4())
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_delete_meal_from_anonymous(self, mock_post):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
        self.create_existing_user_setup()
        data = mock_whatsapp_webhook_data.delete_existing_meal_button_press
        data['entry'][0]['changes'][0]['value']['contacts'][0]['wa_id'] = '11111111111'
        data['entry'][0]['changes'][0]['value']['messages'][0]['from'] = '11111111111'
    ...
    @patch('whatsapp_bot.whatsapp_message_sender.requests.post')
    def test_text_message_from_user(self, mock_post, mock_send_to_lambda):
        mock_post.return_value.json.return_value = {'messages': [{'id': 'mock_message_id'}]}
        self.create_existing_user_setup()
        data = mock_whatsapp_webhook_data.create_meal_for_user_text
        ...
        message = WhatsappMessage.objects.filter(whatsapp_user=self.whatsapp_user).order_by('-record_created_at').first()
        self.assertEqual(message.sent_from, '17204768288')
        ...
        notification_message = WhatsappMessage.objects.filter(whatsapp_user=self.django_whatsapp_user).order_by('-record_created_at').first()
